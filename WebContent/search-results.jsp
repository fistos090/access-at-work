...
<link rel="stylesheet" type="text/css"
	href="css/bidorbuy-stylesheet.css">
...
<body>
	...
	<div class="top-nav">
		<span> <f:formatNumber type="number"
				var="convertedTotalNumOfPages" value="<%= totalNumOfPages %>" />
			Page : [ ${searchResult.getPageNumber()} of <c:out
				value="${convertedTotalNumOfPages}" /> ]
		</span> <a href="index.jsp" class="back-link"> <i
	...
	<div class="container">
		<table class="results-table">
			<tr class="row results-table-head">
				<th colspan="2">bidorbuy trades details: Search results
					<ul class="results-pager">
						<c:if test="${searchResult.getPageNumber() > 1}">
							<li><a href="<%= loadPreviousPgParams %>">Previous</a></li>
						</c:if>
						<c:if
							test="${searchResult.getPageNumber() > 0 && searchResult.getPageNumber() < searchResult.getTotalResults() / searchResult.getResultsPerPage()}">
							<li style="padding-left: 23px"><a
								href="<%= loadNextPgParams %>">Next</a></li>
						</c:if>
					</ul>
				</th>
			</tr>
			<c:forEach varStatus="loop" items="${searchResult.getTrades()}"
				var="tradeProd">
				<tr class="row results-table-body">
					<td class="col-md-3"><img
						src="${tradeProd.getImages()[0].getImageUrl()}"
						alt="${tradeProd.images[0].getThumbnailUrl()}"></td>
					<td class="col-md-9">
						<div class="row">
							<div class="col-md-12">
						...
						<div class="row">
							<span class="col-md-4"> <label for="">Amount</label> <f:formatNumber
									var="amount" type="number" minFractionDigits="2"
									value="${tradeProd.getAmount()}" /> R <c:out value="${amount}" />
							</span> <span class="col-md-4"> <label for="">Opening
									Date</label> <c:out value="${tradeProd.getOpenTime()}" /> <label
								for="">Closing Date</label> <c:out
									value="${tradeProd.getCloseTime()}" />
							</span> <span class="col-md-4"></span>
						</div>
					</td>
				</tr>
			</c:forEach>
